...
// Write to .htaccess for caching
function wpdev_tools_htaccess_write_caching() {
    ...
    $lines[] = 'ExpiresDefault "access 2 days"';
    $lines[] = '</IfModule>';

    insert_with_markers($htaccess, "WPDevToolExpire", $lines);
}


// Write to .htaccess for GZIP
function wpdev_tools_htaccess_write_gzip() {
    $htaccess = get_home_path() . '.htaccess';
    ...
    $lines[] = 'AddType x-font/otf .otf';
    $lines[] = 'AddType x-font/ttf .ttf';
    $lines[] = 'AddType x-font/eot .eot';
    $lines[] = 'AddType x-font/woff .woff';
    $lines[] = 'AddType image/x-icon .ico';
    $lines[] = 'AddType image/png .png';
    
    insert_with_markers($htaccess, "WPDevToolGzip", $lines);
}


// Run write to .htaccess on activation
function wpdev_tools_activation() {
    wpdev_tools_htaccess_write_caching();
    wpdev_tools_htaccess_write_gzip();
}
register_activation_hook(__FILE__, 'wpdev_tools_activation');


// Delete from .htaccess expires headers
function wpdev_tools_htaccess_erase_expire() {
    $htaccess = get_home_path().".htaccess";
...
// Delete from .htaccess Gzip
function wpdev_tools_htaccess_erase_gzip() {
    $htaccess = get_home_path().".htaccess";
...
// Run delete from .htaccess on deactivation
function wpdev_tools_deactivation() {
    wpdev_tools_htaccess_erase_expire();
    wpdev_tools_htaccess_erase_gzip();
}
register_deactivation_hook(__FILE__, 'wpdev_tools_deactivation');
...
function wpdev_tools_modify_jquery() {
    ...
    if(!empty($googlejs)) {
        ...
        wp_register_script('jquery', '//ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js', false, '1.8.1');
